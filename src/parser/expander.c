...
// return expanded var (or $ if it is just $) and moves ptr to next $ occurrence or to the string end or to the next space
static char *eat_var(char **str_ptr)
{
    ...
    if (ft_strchr(*str_ptr + 1, '$') == NULL && ft_strchr(*str_ptr + 1, ' ') == NULL)
    {
